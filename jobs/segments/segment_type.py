...
def combile_operator( segmentField, operator,type, value,):
        if operator == 'EQUAL':
            return f'{segmentField} == {compile_type(type,value)}'
        elif operator == 'GREATER_THAN':
            return f'{segmentField} > {compile_type(type,value)}'
        elif operator == 'LESS_THAN':
            return f'{segmentField} < {compile_type(type,value)}'
        elif operator == 'GREATER_OR_EQUAL':
            return f'({segmentField} > {compile_type(type,value)}) AND ({segmentField} == {compile_type(type,value)})'
        elif operator == 'LESS_OR_EQUAL':
            return f'({segmentField} < {compile_type(type,value)}) AND ({segmentField} == {compile_type(type,value)})'
        elif operator == 'INCLUDES' :
            return f"{segmentField} RLIKE '(^|, ){compile_type(type,value)}($|,)' " # '%compile_type(type,value)%''
        elif operator == 'NOT_EQUAL':
            return f'{segmentField} != {value}'
        elif operator == 'NOT_INCLUDES' :
            return f"{segmentField} NOT RLIKE '(^|, ){compile_type(type,value)}($|,)' "
